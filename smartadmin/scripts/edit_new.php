<?php
	...
	$sql_edit_article = "SELECT * FROM news WHERE news_acc=$edit_id AND lang_id='1'";
	...
	if (mysql_num_rows($query_edit_article)){
		while ($article_row = mysql_fetch_array($query_edit_article))
		{
			...
			$edit_text = stripslashes($article_row['news_text']);
		}
	...
	$sql_edit_article2 = "SELECT * FROM news WHERE news_acc=$edit_id AND lang_id='2'";
	...
	if (mysql_num_rows($query_edit_article2)){
		while ($article_row2 = mysql_fetch_array($query_edit_article2))
		{
			...
			$edit_text_en = stripslashes($article_row2['news_text']);
		}
...
?>
<div class="create_style">Διαχείριση &rsaquo; Νέα &rsaquo; Επεξεργασία νέας είδησης</div>
<br />
<form id="form11" name="edit_article" action="scripts/update_new.php" method="post">
