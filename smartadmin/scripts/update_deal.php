<?php
	...
	if ( (empty($inputed_title) || (empty($inputed_title_en)) || (empty($inputed_body)) || (empty($inputed_body_en)))){
	...
	if (!$valid_entry_err){
			...
			$sql_insert_art = "UPDATE deal_codes SET deal_code_title='$inputed_title' WHERE deal_code_id=$id";
			mysql_query($sql_insert_art) or die('error 1');
			$sql_insert_art = "UPDATE deals SET deal_title='$inputed_title',deal_text='$inputed_body' WHERE deal_acc=$id AND lang_id=1";
			mysql_query($sql_insert_art) or die('error 1');
			
			
			
			$sql_insert_art = "UPDATE deals SET deal_title='$inputed_title_en',deal_text='$inputed_body_en' WHERE deal_acc=$id AND lang_id=2";
	...
	header('Location: ../main.php?action=deals&validation='.$valid_entry);
?>